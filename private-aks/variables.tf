...
variable "node_pool" {
  description = "Node pool configuration options"
  type = map(object({
    node_pool_name           = string
    node_type                = optional(string)
    enable_windows_node_pool = optional(bool)
    min_node_count           = optional(number)
    max_node_count           = optional(number)
    os_sku_windows           = optional(string)
  }))
  default = {
    system = {
...
variable "enable_system_node_pool_unschedule" {
  description = "Pod scheduling options on system node pool"
  type        = bool
  default     = false
}

variable "system_node_pool_taint" {
  description = "The taint to apply on system node pool"
  type        = list(string)
